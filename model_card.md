...
Claude Code was most helpful during the implementation phase — it caught the UserProfile field name mismatch before it became a bug. However I had to double-check its weight experiment suggestions because it initially proposed changes that would have broken the test suite. The moment that surprised me most was seeing that a simple 4-field scoring function could produce results that genuinely feel like personalized recommendations — the math is transparent but the output feels intelligent.
